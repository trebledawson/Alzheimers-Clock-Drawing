...
savefig_ = True
savedir = '.\Results\\' + str(date.today()) + '\multi'
...
def main():
    ...
    for f, d in zip(files, data_files):
        if d in ['_24']:
            for i in range(100):
                print('Dataset:', d[1:])
                print('Test:', i + 1)
                p, t, s, v = train_model_2_class(directory, f, opt, d)
    ...
    with open(savedir + '\mlp-mrmr-' + str(n_features) +
        ...
        print('---------------------------------------------', file=log)
        print('Dataset: 24 (MCI1 vs AD)', file=log)
        print('Number of features:', n_features, file=log)
        print('---------------------------------------------', file=log)
        for p, t, s in zip(performance, train_sizes, test_sizes):
        ...
        avgs = [mean([p[i] for p in performance]) for i in range(3)]
        print('Overall Average Performance (Original Dataset)    :',
              avgs[0], file=log)
        print('Overall Average Performance (SMOTE-NC Before TTS) :',
              avgs[1], file=log)
        print('Overall Average Performance (SMOTE-NC After TTS)  :',
              avgs[2], file=log)

    plt.figure()
    for i in range(len(val_accs)):
        plt.plot(val_accs[i][0])
    plt.title('Original Dataset (No SMOTE)')
    plt.xlabel('Epoch')
    ...
    plt.figure()
    for i in range(len(val_accs)):
        plt.plot(val_accs[i][1])
    plt.title('SMOTE-NC Before Split')
    plt.xlabel('Epoch')
    plt.ylabel('Validation Accuracy')
    plt.grid()
    ...
    plt.figure()
    for i in range(len(val_accs)):
        plt.plot(val_accs[i][2])
    plt.title('SMOTE-NC After Split')
    plt.xlabel('Epoch')
    plt.ylabel('Validation Accuracy')
    plt.grid()
...
def load_data(directory, f, d):
    ...
    d_train_b, y_train_b = SMOTENC(
        ...
        k_neighbors=5).fit_resample(data, labels)
    ...
    d_train_a, y_train_a = SMOTENC(
        categorical_features=cat_cols_idx,
        k_neighbors=5).fit_resample(d_train, y_train)
...
def plot(savedir_, d, tr_loss, tr_acc, vl_loss, vl_acc, best):
    ...
    plt.suptitle('MLP Training History on MCI1 vs AP Dataset, Test ' + str(
        plt.gcf().number))

    if savefig_:
        fig = plt.gcf()
        fig.set_size_inches((11, 8.5), forward=False)
        fig.savefig(fname=(savedir_ + str(fig.number) + '.pdf'),
                    format='pdf',
